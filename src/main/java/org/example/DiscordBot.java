...
public class DiscordBot extends ListenerAdapter {
    ...
    private void sendHelp(MessageReceivedEvent event) {
        ...
        embed.addField("🎯 `!role <nickname>`",
                "AI-powered playstyle analysis across 30 unique archetypes.",
                false);

        embed.addField("📊 `!stats <nickname>`",
                "Full combat dashboard — K/D, ADR, headshots, clutches.",
                false);

        embed.addField("🗺️ `!maps <nickname>`",
                "Map mastery breakdown — exposes best maps and auto-vetoes.",
                false);

        embed.addField("🥊 `!compare <p1> <p2>`",
                "Head-to-head comparison to prove who is better.",
                false);

        embed.addField("🔬 `!advanced <nickname>`",
                "Deep career scan — aces, streaks, lifetime kills, MVPs.",
                false);

        embed.addField("🏆 `!leaderboard`",
                "View the server leaderboard ranked by ELO.",
                false);

        embed.addField("ℹ️ `!info`",
                "Meet Vitaly and hear what the bot is all about.",
                false);
    ...
    private void sendLeaderboard(MessageReceivedEvent event) {
        ...
        if (topPlayers.isEmpty()) {
            ...
            event.getChannel().sendMessage(
                    "📭 The leaderboard is empty! Scan players with `!stats` or `!role` first."
            ).queue();
    ...
    private void sendStats(MessageReceivedEvent event, String nickname) {
        ...
        embed.addField(
                "🔫 AIM & FRAGGING",
                dot(stats.getKd(), 1.0, 1.3) +
                        " **K/D** `" + String.format("%-5.2f", stats.getKd()) + "` " +
                        bar(stats.getKd(), 2.0) + "\n" +

                        dot(stats.getAdr(), 70, 90) +
                        " **ADR** `" + String.format("%-5.1f", stats.getAdr()) + "` " +
                        bar(stats.getAdr(), 120.0) + "\n" +

                        dot(stats.getHs(), 45, 60) +
                        " **Headshot** `" + String.format("%-4.1f", stats.getHs()) + "%` " +
                        bar(stats.getHs(), 80.0),
                false
        );
    ...
    private void sendRole(MessageReceivedEvent event, String nickname) {
        ...
        embed.setFooter(
                "Requested by " + event.getAuthor().getName() + " • Vitaly AI",
    ...
    private void sendAdvancedStats(MessageReceivedEvent event, String nickname) {
        ...
        embed.addField(
                "☠️ MULTI-KILL HIGHLIGHTS",
                "🔴 **Aces (5K)** — `" + stats.getAces() + "`\n" +
                        "🟠 **Quad (4K)** — `" + stats.getQuadKills() + "`\n" +
                        "🟡 **Triple (3K)** — `" + stats.getTripleKills() + "`",
                true
        );
    ...
    private void sendMaps(MessageReceivedEvent event, String nickname) {
        ...
        embed.addField(
                "👑 Best Map: " + best1.getCleanName(),
                "**Win Rate:** " + best1.getWinRate() +
                        "%\n**K/D Ratio:** " + best1.getKd() +
                        "\n**Matches:** " + best1.getMatches(),
                true
        );
        ...
        if (validMaps.size() > 1) {
            ...
            embed.addField(
                    "🥈 Second Best: " + best2.getCleanName(),
                    "**Win Rate:** " + best2.getWinRate() +
                            "%\n**K/D Ratio:** " + best2.getKd() +
                            "\n**Matches:** " + best2.getMatches(),
                    true
            );
        }
        ...
        embed.addField(
                "🗑️ Auto-Veto: " + worst1.getCleanName(),
                "**Win Rate:** " + worst1.getWinRate() +
                        "%\n**K/D Ratio:** " + worst1.getKd() +
                        "\n**Matches:** " + worst1.getMatches(),
                true
        );
        ...
        if (validMaps.size() > 2) {
            ...
            embed.addField(
                    "⚠️ Weak Link: " + worst2.getCleanName(),
                    "**Win Rate:** " + worst2.getWinRate() +
                            "%\n**K/D Ratio:** " + worst2.getKd() +
                            "\n**Matches:** " + worst2.getMatches(),
                    true
            );
        }
